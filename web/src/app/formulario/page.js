...
export default function FormPageOne() {
  ...
  const validateForm = () => {
    if (!firstName.match(/^[A-Za-zÑñ]+$/)) {
      alert('Solo letras para nombre');
      //la ñ no es valido
    } if (!lastName.match(/^[A-Za-zÑñ]+$/)) {
  ...
  return (
    <div class="flex bg-violet-university justify-center justify-items-center align-middle">
      <div
        className=" w-2/3 p-6 m-8 bg-white rounded-md shadow-md text-violet-university"
      >

        <h2 className="text-3xl  font-bold mb-4">Formulario</h2>
